...
For a wff \script{A}, a constant $c$, and a variable $x$, define \script{A}\substitute{x}{c} to mean the wff that we get by replacing every occurrence of $x$ in \script{A} with $c$.

\script{A}\substitute{x}{c} is called a \define{substitution instance} of $\forall x\script{A}$ and $\exists x\script{A}$, and $c$ is called the \define{instantiating constant}.
...
If you have $\forall x Ax$, it is legitimate to infer that anything is an $A$. You can infer $Aa$, $Ab$, $Az$, $Ad_3$. This is, you can infer any substitution instance -- in short, you can infer $A\script{c}$ for any constant \script{c}. This is the general form of the universal elimination rule ($\forall$E):

\begin{proof}
	\have[m]{a}{\forall \script{x}\script{A}}
	\have[\ ]{c}{\script{A}\substitute{x}{c}} \Ae{a}
\end{proof}


Remember that the box mark for a substitution instance is not a symbol of QL, so you cannot write it directly in a proof. Instead, you write the substituted sentence with the constant \script{c} replacing all occurrences of the variable \script{x} in \script{A}. For example:
...
\begin{proof}
	\have[m]{a}{\script{A}}
	\have[\ ]{c}{\exists \script{x}\script{A}\substitutesome{x}{c}} \Ei{a}
\end{proof}

It is important to notice that \script{A}\substitutesome{x}{c} is not necessarily a substitution instance. We write it with double boxes to show that the variable \script{x} does not need to replace all occurrences of the constant \script{c}. You can decide which occurrences to replace and which to leave in place. For example:
...
\begin{proof}
	\have[m]{a}{\script{A}}
	\have[\ ]{c}{\forall \script{x}\script{A}\substitute{c^\ast}{x}} \Ai{a}
\end{proof}
...
\begin{quote}
Since $\exists x Sx$, there is something that is an $S$. We do not know which constants refer to this thing, if any do, so call this thing `Ishmael'. From $\forall x(Sx \eif Tx)$, it follows that if Ishmael an $S$, then it is a $T$. Therefore, Ishmael is a $T$.  Because Ishmael is a $T$, we know that $\exists x Tx$.
\end{quote}
...
We want the existential elimination rule to work in a similar way. Yet since English language worlds like `Ishmael' are not symbols of QL, we cannot use them in formal proofs. Instead, we will use constants of QL which do not otherwise appear in the proof.
...
\begin{proof}
	...
	\open	
		\hypo[n]{b}{\script{A}\substitute{c^\ast}{x}}
		\have[p]{c}{\script{B}}
	\close
...
Remember that the proxy constant cannot appear in \script{B}, the sentence you prove using $\exists$E.

It would be enough to require that the proxy constant not appear in $\exists\script{x}\script{A}$, in \script{B}, or in any undischarged assumption. In recognition of the fact that it is just a place holder that we use inside the subproof, though, we require an entirely new constant which does not appear anywhere else in the proof.
...
Notice that this has effectively the same structure as the English-language argument with which we began, except that the subproof uses the proxy constant `$a$' rather than the bogus name `Ishmael'.
