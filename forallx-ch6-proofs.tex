...
Logical proofs are called \emph{derivations}. So `derive' means the same thing as `prove'.
...
Working with proofs allows us to define a number of related concepts. The most basic of these is \emph{provability}. If it is possible to write a proof which assumes \script{A} and concludes \script{B}, then we say `\script{B} is provable from \script{A}.' We abbreviate this $\script{A}\vdash\script{B}$.

Since that `derive' and `prove' are synonyms, $\script{A}\vdash\script{B}$ can also be read as as `\script{B} is derivable from \script{A}.'

The `$\vdash$' symbol is called the \emph{turnstile}. Sometimes it is called a \emph{single turnstile}, to underscore the fact that this is not the {double turnstile} symbol ($\models$) that we used to represent semantic entailment in ch.~\ref{ch.semantics}.

For proofs with more than one assumption, we can put a set of sentences on the lefthand side of the turnstile. So $\{\script{A}_1,\script{A}_2,\ldots\}\vdash\script{B}$ means that it is possible to give a proof of \script{B} with $\script{A}_1$,$\script{A}_2$,$\ldots$ as premises.

For proofs with no assumptions, we write nothing on the lefthand side of the turnstile. So $\vdash\script{C}$ means that there is a proof of \script{C} that has no premises.

\subsection{Theorems}

We define a \define{theorem} as a sentence that is derivable without any premises; i.e., \script{T} is a theorem if and only if $\vdash\script{T}$.

It is not too hard to show that something is a theorem. You just have to give a proof of it.

But how could you show that something is \emph{not} a theorem? If its negation is a theorem, then you could provide a proof. For example, it is easy to prove $\enot(Pa \eand \enot Pa)$, which shows that $(Pa \eand \enot Pa)$ cannot be a theorem. For a sentence that is neither a theorem nor the negation of a theorem, however, there is no easy way to show this. You would have to demonstrate not just that certain proof strategies fail, but that no proof is possible. Even if you fail in trying to prove a sentence in a thousand different ways, perhaps the proof is just too long and complex for you to make out.
...
It is easy to show that a set is provably inconsistent. You just need to assume the sentences in the set and prove a contradiction.

Showing that a set is \emph{not} provably inconsistent would be much harder. It would require more than just providing a proof or two. Instead, it would require showing that proofs of a certain kind are \emph{impossible}.
...
Given that the proof system is sound, it follows that every theorem is a tautology.
